				...
				if ($.isArray(items)) {
					...
					$.each(items, function (i, opt) {
						if (typeof valuesHash[opt.id] === 'undefined') {
							$('<option style="background:url(' + opt.foto_perfil + '); background-size: 32px 32px !important; background-repeat:no-repeat !important; vertical-align:middle; width:auto; height:32px; padding-left:45px !important; color:#333 !important; padding-top:10px; background-position: top 2px left 8px; font-size:14px; font-family:Helvetica Neue, Helvetica, Arial, sans-serif; font-weight:normal !important;" value="' + opt.id + '">' + opt.text +' '+ opt.sobre_nome +'</option>').appendTo(select);
						}
					});
				} else {
