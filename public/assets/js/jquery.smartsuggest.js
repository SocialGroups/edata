										...
										if (options.showEmptyCategories || (!options.showEmptyCategories && group['data'].length != 0))
										{
											...
											$.each(group['data'], function (j, item) {
												if (count < limit)
												{
													...
													output += (item['image'] != undefined && options.showImages) ? '<div class="resultImg"><img src="'+item['image']+'" /></div>'+"\n" : '';
													output += (item['primary'] != undefined) ? '<span class="resultTitulo">'+item['primary']+"</span><br />\n" : '';
													output += (item['secondary'] != undefined) ? '<div class="resultDescricao">'+item['secondary']+'</div><br/>'+"\n" : '';
													output += '<td>';
													output += '<p>';
